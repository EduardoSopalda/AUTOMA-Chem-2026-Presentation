...
## 5e. The visual bar (27 September)

The first full run through looked like a wireframe next to the cover. Edu stopped the build. This overrides the SVG first approach in sections 6 and 7, and the rejection of generated imagery in 5b.

* **The bar is the cover artwork** (brand bible, page 1): dense, layered, hand drawn blueprint and copper on textured paper. Every frame must look like it belongs to the same set.
* **Method:** one rich raster illustration per act (about nine), 3840 wide, in the cover's style, generated by Edu from the brief in `docs/ILLUSTRATIONS.md`. Clicks reveal regions of them with masks and wipes. SVG only for the lines that must move: dimension line, section cuts, the Act 1 column, the Congresso.
* **The figure** is the drawn man from the cover, cut out at high resolution. No pictogram.
* **Text:** half as much on screen. Serif statements stay; most small sans labels go.
